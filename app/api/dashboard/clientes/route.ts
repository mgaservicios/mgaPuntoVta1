...
export async function POST(req: NextRequest) {
  ...
  const { nombre, tipo = 'PARTICULAR', email, telefono, direccion, localidad, cuit, notas } = body
  ...
  const { data, error } = await supabase
    .from('clientes')
    .insert({ nombre: nombre.trim(), tipo, email, telefono, direccion, localidad, cuit, notas })
    .select()
    .single()

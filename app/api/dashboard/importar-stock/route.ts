...
export async function POST(req: NextRequest) {
  ...
  const { rows } = await req.json() as { rows: { codigo: string; stock: number }[] }
  if (!Array.isArray(rows) || rows.length === 0)
    return NextResponse.json({ error: 'Sin filas' }, { status: 400 })

  // ── Proveedor "Stock Inicial" — buscar o crear ─────────────────────────────
  let proveedorId: number
  const { data: provExistente } = await supabase
    .from('proveedores')
    .select('id')
    .ilike('nombre', 'Stock Inicial')
    .limit(1)
    .single()

  if (provExistente) {
    proveedorId = provExistente.id as number
  } else {
    const { data: provNuevo, error: provErr } = await supabase
      .from('proveedores')
      .insert({ nombre: 'Stock Inicial', activo: true })
      .select('id')
      .single()
    if (provErr || !provNuevo)
      return NextResponse.json({ error: 'No se pudo crear el proveedor Stock Inicial' }, { status: 500 })
    proveedorId = provNuevo.id as number
  }

  const errors: { codigo: string; error: string }[] = []

  // ── Batch-lookup articulo_ids por codigo ───────────────────────────────────
  const codigos = [...new Set(rows.map(r => r.codigo?.trim()).filter(Boolean))]
  ...
  // ── Procesar en batches de ITEMS_POR_REMITO ────────────────────────────────
  for (let i = 0; i < items.length; i += ITEMS_POR_REMITO) {
    ...
    const { data: remito, error: errRemito } = await supabase
      ...
      .insert({
        ...
        contraparte_proveedor_id: proveedorId,
        contraparte_nombre: 'Stock Inicial',
        fecha: new Date().toISOString(),
        observaciones: 'Importación de stock',
        estado: 'borrador',
    ...
    // ── Sincronizar articulos.stock_actual (total multi-sucursal) ─────────

...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: rpcError } = await tenantClient.rpc('reset_precios')

    if (rpcError) {
      const detail = rpcError.message.includes('schema cache')
        ? `La función reset_precios no existe en la BD del tenant. Aplicar la migración 20260723_reset_stock_precios.sql en el proyecto Supabase de "${empresa.nombre}".`
        : `Error al limpiar precios: ${rpcError.message}`
      return NextResponse.json({ error: detail }, { status: 500 })
    }

    return NextResponse.json({
      ok: true,
      message: `Precios de "${empresa.nombre}" (${empresa.codigo}) limpiados exitosamente`,
    })
  } catch (err) {
    return NextResponse.json(
